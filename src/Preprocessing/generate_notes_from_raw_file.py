...
def remove_date_dashes(event_date):
    ...
    event_date = event_date.replace(":","")
    event_date = event_date.replace(" ", "")
    return event_date.split()[0] # theres an hour stamp im cutting off by doing this. no theres not. i need those hour nums
    ...
    for date_num,blob in blobs2:
        ...
        if len(metadata_and_text) > g.BLOB_FIELDS:
            ...
            if re.match(r"\D*(\d{10})\D*", clinical_event_id):
                ...
                ############################
                # Special case for doc id troiseptquatrequatre (he’s got three MRNs
                #
                with open(c.NOTE_OUTPUT_DIR + first_id + "_" + get_special_doc_id(count), "w") as writefile: # was clinical_event_id, not count
                ...
                if(first_id in caisis_silver_dict):
                    patient_silver_file.write(first_id + " " + get_caisis_silver_labels(caisis_silver_dict[first_id]) + "\n")
                else:
                    patient_silver_file.write(first_id + " UNKNOWN" + "\n")

                # # NoNeedTODO: figure out how to write a document level gold doc
                # write_doc_level_gold_file(first_id, get_special_doc_id(count))
                #
                # # NoNeedTODO: figure out how to write the patient level gold doc
